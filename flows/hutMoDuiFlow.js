...
async function sendHutMoDuiFlow(sender_psid) {
  console.log("🚀 Trigger Hút Mỡ Đùi Flow");
  ...
  const feedbackImages = await getFeedbackImages("hutmodui");
  const maxImages = 7;
  ...
  const bangGiaImage = await getBangGiaImage("banggia_hutmodui");
  if (bangGiaImage) {
    await messengerClient.sendMessage(sender_psid, {
