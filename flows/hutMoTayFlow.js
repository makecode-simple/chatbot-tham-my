...
async function sendHutMoTayFlow(sender_psid) {
  console.log("🚀 Trigger Hút Mỡ Tay Flow");
  ...
  const feedbackImages = await getFeedbackImages("hutmotay");
  const maxImages = 7;
  ...
  const bangGiaImage = await getBangGiaImage("banggia_hutmotay");
  if (bangGiaImage) {
    await messengerClient.sendMessage(sender_psid, {
